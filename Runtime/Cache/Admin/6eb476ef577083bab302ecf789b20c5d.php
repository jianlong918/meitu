            ...
            <div class="col-sm-6" style='width: 100%'>
                <div class="ibox">
                    <div class="ibox-title">
                        <h5>广告管理</h5>
                        <?php if($auth->check( 'admin/ads/add', session('uid') )){ ?>
                        <a href="<?php echo U('admin/ads/add');?>" class="btn btn-info btn-xs" style='margin-left: 20px'>添加</a>
                        <?php } ?>
                    </div>
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th>排序</th>
                                    <th>图片</th>
                                    <?php if($auth->check( 'admin/ads/update', session('uid') )||$auth->check( 'admin/ads/del', session('uid') )){ ?>
                                    <th>操作</th>
                                    <?php } ?>
                                </tr>
                            </thead>
                            <tbody>
                                 <?php if(is_array($data["list"])): $i = 0; $__LIST__ = $data["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeA">
                                            ...
                                            <td><?php echo ($vo["position_sn"]); ?></td>
                                            <td><?php echo ($vo["url"]); ?></td>
                                            <td><?php echo ($vo["sortrank"]); ?></td>
                                            <td><img src="<?php echo ($vo["img"]); ?>" height="150px;"></td>
                                            <?php if($auth->check( 'admin/ads/update', session('uid') )||$auth->check( 'admin/ads/del', session('uid') )){ ?>
                                            <td class="center">
                                                <?php if($auth->check( 'admin/ads/update', session('uid') )){ ?>
                                                <a href="/admin/ads/update?id=<?php echo ($vo["id"]); ?>" class="badge badge-primary">修改</a>&nbsp;&nbsp;
                                                <?php } ?>
                                                <?php if($auth->check( 'admin/ads/del', session('uid') )){ ?>
