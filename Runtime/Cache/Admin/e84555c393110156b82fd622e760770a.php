            ...
            <div class="col-sm-6" style='width: 100%'>
                <div class="ibox">
                    <div class="ibox-title">
                        <h5>文章管理</h5>
                        <?php if($auth->check( 'admin/newsarticle/add', session('uid') )){ ?>
                        <a href="<?php echo U('admin/newsarticle/add');?>" class="btn btn-info btn-xs" style='margin-left: 20px'>添加</a>
                        <?php } ?>
                        <?php if($auth->check( 'admin/newsarticle/del_many', session('uid') )){ ?>
                        <a href="javascript:void(0)" onclick="confirm_del_many(1)" class="btn btn-danger btn-xs" style='margin-left: 20px'>删除</a>
                        <?php } ?>
                    </div>
                    ...
                    <div class="ibox-content">
                        ...
                        <table class="table table-bordered">
                            ...
                            <?php if(is_array($data["list"])): $i = 0; $__LIST__ = $data["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="odd gradeA">
                                    ...
                                    <?php if($auth->check( 'admin/newsarticle/update', session('uid') )||$auth->check( 'admin/newsarticle/del', session('uid') )){ ?>
                                    <td class="center">
                                        <?php if($auth->check( 'admin/newsarticle/update', session('uid') )){ ?>
                                        <a href="/admin/newsarticle/update?id=<?php echo ($vo["id"]); ?>" class="badge badge-primary">修改</a>&nbsp;&nbsp;
                                        <?php } ?>
